-- Fix get_gallery_events to only return future events
-- Adds date filter to match search_events behavior (only events after 2025-10-14)
-- Changes ordering from DESC (past first) to ASC (upcoming first)
...
BEGIN
  ...
  ORDER BY e.start_at ASC NULLS LAST  -- Upcoming events first (was DESC)
  LIMIT event_limit;
END;
$$;

COMMENT ON FUNCTION get_gallery_events IS 'Get upcoming events for a specific gallery. Filters to only show events after 2025-10-14, ordered by start date (soonest first).';
