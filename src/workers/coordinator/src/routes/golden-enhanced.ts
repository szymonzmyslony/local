...
export async function getGoldenEntities(
  ...
  const singularEntityType: EntityType =
    entityType === "artists"
      ? "artist"
  ...
  const { data, error, count } = await queryGoldenEntities(
    sb,
    singularEntityType
  )
    .select("*", { count: "exact" })
...
export async function approveCluster(
  ...
  const { data: goldenData, error: goldenError } = await queryGoldenEntities(
    sb,
    entityType
  )
    .insert(goldenRecord)
  ...
  if (updateError) {
    ...
    console.error(
      "Failed to update merge_history approval status:",
      updateError
    );
  }
