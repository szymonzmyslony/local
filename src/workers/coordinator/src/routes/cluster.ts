...
import { queryExtractedEntities, getGoldenTableName } from "../lib/table-helpers";
...
export async function previewCluster(
	...
	const { data: entities, error } = await queryExtractedEntities(sb, body.entity_type)
		.select("*")
	...
	const preview: Record<string, Array<{ entity_id: string; value: unknown }>> = {};
...
export async function commitCluster(
	...
	if (
		...
		return jsonResponse(400, {
			error:
				"entity_type, entity_ids, and field_selections are required",
		});
	}
	...
	const { error: updateError } = await queryExtractedEntities(sb, body.entity_type)
