...
export function SimilarityTab() {
  ...
  const dismissMutation = useMutation({
    mutationFn: async (linkId: string) => {
      const res = await fetch(
        `/api/similarity/pairs/${linkId}/${entityType}/dismiss`,
        {
          method: "POST",
  ...
  const mergeMutation = useMutation({
    mutationFn: async (linkId: string) => {
      const res = await fetch(
        `/api/similarity/pairs/${linkId}/${entityType}/merge`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({})
    ...
    <div className="space-y-4 p-6">
      <div>
        ...
        <div className="flex items-center gap-4 mb-6">
          <CrawlJobSelector
            value={crawlJobFilter}
            onChange={setCrawlJobFilter}
          />
          ...
          <div className="flex items-center gap-2 min-w-[300px]">
            ...
            <Slider
              ...
              onValueChange={(values) =>
                setSimilarityRange(values as [number, number])
              }
              min={0.5}
              max={1.0}
              step={0.01}
            ...
            <span className="text-sm text-gray-600 min-w-[100px]">
              {(similarityRange[0] * 100).toFixed(0)}% -{" "}
              {(similarityRange[1] * 100).toFixed(0)}%
            </span>
          </div>
        </div>
            ...
            {isLoading ? (
              ...
              <div className="flex items-center justify-center p-12 border rounded-lg bg-gray-50">
                <div className="text-center text-gray-500">
                  ...
                  <p className="text-sm mt-2">
                    {crawlJobFilter
                      ? "Try selecting a different crawl job or"
                      : "Try"}{" "}
                    adjusting the similarity range
                  </p>
                </div>
              </div>
            ) : (
              <div className="space-y-4">
                <div className="text-sm text-gray-600">
                  {pairs.length} similar {pairs.length === 1 ? "pair" : "pairs"}{" "}
                  found
                ...
                {pairs.map((pair) => {
                  const nameA =
                    pair.source_a_name || pair.source_a_title || "Unnamed";
                  const nameB =
                    pair.source_b_name || pair.source_b_title || "Unnamed";
                  const bioA =
