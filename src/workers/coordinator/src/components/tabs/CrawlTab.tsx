...
export function CrawlTab() {
	...
	const { data: urlsData, isLoading: urlsLoading } = useQuery({
		...
		queryFn: async () => {
			...
			return response.json() as Promise<{ urls: DiscoveredUrl[]; total: number }>;
		},
		enabled: !!expandedJob,
	});
		...
		<div className="space-y-6">
			...
			<Card className="p-6">
				...
				{jobsLoading ? (
					...
					<div className="text-neutral-600 dark:text-neutral-400">No crawl jobs yet</div>
				) : (
							...
							<div key={job.id}>
								...
								{expandedJob === job.id && (
									<Card className="mt-2 ml-4 p-4 bg-neutral-50 dark:bg-neutral-900">
										...
										{urlsLoading ? (
											<div className="text-neutral-600 dark:text-neutral-400 text-sm">Loading...</div>
										) : !urlsData?.urls.length ? (
											<div className="text-neutral-600 dark:text-neutral-400 text-sm">No URLs discovered yet</div>
										) : (
												...
												{urlsData.urls.map((url) => (
													...
													>
														...
														<span className={`ml-2 text-xs ${
															url.status === 'fetched' ? 'text-green-600 dark:text-green-400' :
															url.status === 'failed' ? 'text-red-600 dark:text-red-400' :
